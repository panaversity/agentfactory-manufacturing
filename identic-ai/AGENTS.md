...
- **Bring-your-own is one env var.** A reader who has their own deployed Paperclip points the MCP at it by setting `PAPERCLIP_API_KEY` to a **board-scoped credential** (the approval-decision routes are board-gated, so the credential Claudia acts through must be a board credential, not a Paperclip agent key). Same architecture, no parallel mock to maintain.
...
- **Board key vs agent key, and why Claudia is not a registered Paperclip agent.** The three approval-decision routes are `assertBoard`-gated: only a credential acting **as the board** (local-trusted mode in the sandbox, no key, or a board API key) can approve or reject. A **Paperclip agent** credential (`agent_api_keys`, `PAPERCLIP_API_KEY`) **cannot decide approvals at all**: those routes reject an agent key with 403. So Claudia acts purely through the **board path**, and she is **not registered as a Paperclip agent** at all: an agent registration would buy her nothing here (it cannot approve), so there is no second "Claudia" record in Paperclip and no agent key to mint or revoke. The CEO and the workforce ARE registered Paperclip agents, because they do company work; Claudia sits **above** the company, at the edge, as the owner's delegate, and reaches Paperclip only to resolve board approvals. Her identity and attribution are her **ed25519 signature plus the local ledger**, not a Paperclip nameplate. Do not try to make an agent key approve, and do not register the delegate as one; both are dead ends by design.
...
- **Paperclip** (CalVer `YYYY.MMDD.patch`, re-pin before any cohort): MCP tools `paperclipListApprovals` / `paperclipGetApproval` / `paperclipApprovalDecision` / `paperclipCreateApproval`; `paperclipApprovalDecision` takes `{ approvalId, action: "approve"|"reject"|"requestRevision"|"resubmit", decisionNote?, payloadJson? }` and POSTs to the board-scoped routes. The MCP env is `PAPERCLIP_API_URL` (bare host; the server appends `/api`) + `PAPERCLIP_API_KEY` (**required to boot, even for the sandbox**: the client throws `Missing PAPERCLIP_API_KEY` if unset; the sandbox accepts any non-empty placeholder like `local-trusted` and does not validate it; bring-your-own needs a real **board-scoped** key, since Claudia acts on the board-gated approval routes). `paperclipai onboard --yes` = local-trusted sandbox (keyless routes). Agent lifecycle / key routes (`DELETE /api/agents/:id/keys/:keyId`, `pause`/`resume`/`terminate`) apply to the **CEO and workforce agents**, not to Claudia (she is not a registered Paperclip agent); her off-switch is stopping her loop and rotating her board credential.
...
6. **The week payoff (you do NOTHING; her loop does the work, then briefs you).** Seed the approval flood from `seed-company.json`'s `demo_approval_flood` into the sandbox (the CEO proposes hires; refunds land across the bands). Then wait for a heartbeat, or trigger one with `openclaw cron run <id> --wait`. Her loop clears the in-envelope refunds (signed + posted + logged), surfaces the hires and the out-of-band refunds to the owner, and writes a ledger row for each. Then, before she sleeps, she composes and pushes the owner a one-line brief of that pass: cleared N for $X, M need you with the titles, the company in a sentence (the `decide.mjs --scan` brief step, on the paired channel). Read `~/.openclaw/governance/ledger.jsonl` to see her work and the owner channel to see the brief; this is the proof that a delegate cleared the week AND reported it. The heartbeat governs the queue AND briefs the owner; it is not just a clearing loop. The envelope HARD-REFUSES any out-of-band attempt at the act step, independent of her reasoning. (A separate standing daily brief, if the owner wants one, is `decide.mjs --brief-only`: it summarizes the current queue and sends it without deciding or writing the ledger.)
7. **Recovery (stolen laptop / device switch).** Claudia is NOT a registered Paperclip agent, so there is no agent key to revoke; that was never what gated her acting. The real off-switch is to **stop her loop and cut the credential she acts through**: `openclaw gateway stop` (or `openclaw cron` disable her heartbeat) so she stops waking and posting, and **rotate the board credential** she uses to clear approvals so a stolen copy can no longer post in your name. Then copy `~/.openclaw/` to a new device, reissue her credentials there, and restart her heartbeat; her judgment survives in her files. Stopping the loop and rotating the board credential must require the owner-human, never a delegate signature.
...
- **Brief (the chief-of-staff payoff):** after the pass, the owner channel carries a one-line `Heartbeat brief:` summary composed from the real pass (cleared count + dollars, the surfaced items by title, the company in a sentence), not just the per-item surface messages. The brief is a function of the tally, so seeding a different flood produces a different brief.
...
- **Recovery:** stopping her loop and rotating the board credential she acts through blocks further posts in the owner's name (there is no Paperclip agent key to revoke, since she is not registered as one); the owner's accumulated judgment in `~/.openclaw/` survives the device move when backed up.
