...
You are the coding agent (Claude Code, OpenCode, Codex, or similar). The human you are paired with is the owner. They decide _what_ to delegate; you handle _how_ to stand it up. You verify OpenClaw (the owner installed it in the on-ramp; you reinstall only if it is absent), place the pre-authored delegate ("Claudia") who acts on the owner's behalf, wire the official Paperclip MCP and a Neon governance ledger, and build the `governance-ledger` skill on top of the shipped `sign-decision` skill. You do the real setup and verification, not just code generation.
...
The course's premise is an AI-native company (yours, carried forward from Courses 5-7) whose owner cannot scale their own attention: every consequential Worker action routes to one human's approval gate, and approval volume grows with the workforce while the owner's hours do not. The delegate you build resolves the routine slice of that gate inside an owner-set envelope and surfaces the rest.
...
- The agent loads `seed-company.json` (your four Workers: Tier-1 Support, Tier-2 Specialist, Manager-Agent, Legal Specialist) and `course-seven-export/approvals.json` (your prior decisions, the judgment seed) into that sandbox. A reader with no upstream stack has a gradable company on day one.
...
The build runs in **two acts**. **Act 1 builds the twin**: you place Claudia's shipped workspace and prove she reasons like the owner, wired to no company. **Act 2 stands up the company** (Paperclip) and gives Claudia a bounded mandate to govern its approval queue. The company is NOT part of the twin build; it does not appear until Act 2. Read the page section for each scenario; plan, then build and verify.
...
1. **Verify OpenClaw, then place Claudia's workspace.** Claudia is shipped pre-authored in `claudia-workspace/` (`IDENTITY.md`, `SOUL.md`, `USER.md`, `AGENTS.md`, `TOOLS.md`, `HEARTBEAT.md`, `memory/`, `skills/sign-decision/`); you place it, you do not write a persona from scratch.
   ...
   - **Place Claudia:** copy `claudia-workspace/`'s contents into `~/.openclaw/workspace/`, then run `openclaw onboard --skip-bootstrap` so onboard does NOT generate its own brain files and the shipped ones win. (`--skip-bootstrap` is docs-confirmed but verify live; the documented config-setting fallback is `{ agents: { defaults: { skipBootstrap: true } } }` in `~/.openclaw/openclaw.json`. Re-running onboard does NOT wipe unless `--reset`.)
