...
⚠️ **Edge status (verify before relying):** CIMD is an IETF _draft_ (`draft-ietf-oauth-client-id-metadata-document`), not broadly implemented — Better Auth does **not** support CIMD yet (DCR only), and FastMCP has it as an open issue (SEP-991). So for a runnable course today use static or DCR, and teach CIMD as where the standard is heading. Re-check at ship time; this is moving fast.
