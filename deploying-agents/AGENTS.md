...
- **Strip `channel_binding` from the Neon DSN before asyncpg.** Neon's copy-paste string includes `channel_binding=require`. asyncpg does not recognize it and treats it as a server setting, which fails against the pooler. Keep `sslmode=require`, drop `channel_binding`.
