...
- ⚠️ **Verified gotcha (1.7.0-rc.0, when the `oauthProvider` from specs 02-04 is also present):** the `jwt.issuer` line above is **redundant** (the plugin already defaults `iss` to `BETTER_AUTH_URL`) **and** collides with the oauthProvider's own issuer, which **404s the OIDC discovery document** — silently breaking specs 02-04. **Omit `jwt.issuer`**; keep `jwt.audience`. You still get `iss = BETTER_AUTH_URL` on the token and a healthy discovery doc.
