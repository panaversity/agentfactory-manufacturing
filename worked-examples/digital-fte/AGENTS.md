...
Three invariants a fresh session must follow (the full reasoning is in "Rules that prevent silent failures" below):

1. **Business data goes through the `customer-data` MCP server only.** The running Worker never issues raw SQL against business data — every read and write is one of that server's scoped tools.
2. **Audit uses its own direct database connection, and each action commits with its audit row.** The audit layer connects directly (its own `asyncpg` pool, not the MCP boundary it audits); a state-changing action and its `audit_log` row commit together or not at all.
3. **Embeddings use one model for both store and search.** The same embedding model (and dimension) writes the vectors and runs the queries, or results are nonsense.
...
- **Install the right extras, or the import fails.** `SQLAlchemySession` lives behind an extra: `uv add "openai-agents[sqlalchemy]"`. The bare `openai-agents` raises `ImportError` ("requires the 'sqlalchemy' extra") the moment you import it. Part 4 also needs `uv add asyncpg pgvector` (the `pgvector` package is what `from pgvector.asyncpg import register_vector` imports). The Quick Win needs only `openai-agents`. Verified live against `openai-agents` 0.17.3.
- **State persists through the SDK Session, on Postgres.** Back the conversational `Session` with `SQLAlchemySession.from_url(session_id, url=<the asyncpg URL of this Neon DB>, create_tables=True)`; it auto-stores every turn. Two flavours, and you choose: **default** is to let the Session own the turns and keep `conversations` for business metadata and the closing summary (no hand-rolled `messages` table, no double-store); build a **custom** turns table only when you need per-turn rows you can query, type, or embed directly, and then that table IS the store, not a duplicate of the Session. The conversational `session=` on `Runner.run` is NOT the `SandboxRunConfig` sandbox session; never conflate the two.
