...
**The hire-approval gate (`requireBoardApprovalForNewAgents`) must be set by PATCH, not at create (verified live, 2026.529.0).** If the human wants to sign off on every hire, you cannot just pass `requireBoardApprovalForNewAgents: true` in the create body: that field is **silently dropped on company create** (you send `true`, the stored value is `false`, confirmed on two fresh companies). Enable it with a follow-up call once the company exists: `PATCH /api/companies/:id` with `{"requireBoardApprovalForNewAgents": true}` (verified: the PATCH persists `true`). The flag defaults to **false**, and while it is false the CEO hires its reports **directly, with no `hire_agent` approval**. So when the human asks to control hiring (Scenario 1), the PATCH is not optional cleanup; it is the step that arms the gate the course's Scenario 4 depends on. Skip it and the CEO will staff the company with no sign-off, while the dashboard still says you are in control. After the PATCH, read the company back and confirm the flag is `true` before you tell the human the gate is on.
...
The LLM-runtime adapters (`claude_local`, `codex_local`, `gemini_local`, `opencode_local`, and the rest) drive a real model. **These are what the crash course uses**: the CEO and its reports must reason, so they run on a model adapter. Pick the one that matches whatever coding agent the human already has logged in (`claude_local` for Claude Code, `opencode_local` for OpenCode, and so on); `claude_local` is the default starting point only because it is the most common. Verified live: they authenticate through the machine's own agent-CLI login, so no env key is required; and on those logins Paperclip records their tokens but at `cost_cents` 0, so the budget reads $0 (see Budgets).

**Per-adapter model trap (verified live, 2026.529.0).** `claude_local` is safe with `model` omitted (it uses the local default). **`opencode_local` is NOT**: a fresh `opencode_local` agent auto-defaulted to `openai/gpt-5.2-codex`, which did not exist in that OpenCode install, so the heartbeat failed `adapter_failed` ("Configured OpenCode model is unavailable: ... Available models: ...") and Paperclip escalated the task to `blocked`. The failure is visible (clear error in the run log and an issue comment), not silent, but it does block the work. So for `opencode_local`, do not trust the auto-default: set `adapterConfig.model` to a real id from the install's own list (the error prints `Available models: ...`; e.g. `openai/gpt-5.4-mini` or `openai/gpt-5.3-codex` worked, taking the task to `done` keyless). General rule: omit `model` for `claude_local`; pass an explicit, install-valid `model` for `opencode_local` (and confirm any id before relying on it, the way the `claude_local` invalid-id trap already taught us).
...
- **`reportsTo`** is the org-chart link: the agent id of this agent's manager (**verified live, 2026.529.0**). The CEO is created with `reportsTo` **omitted/null** (it reports to the human, the board). Every later hire must set `reportsTo` to the CEO's agent id (or to a manager below the CEO once the org deepens). Skip it and the report is created flat, reporting to no one, which silently breaks the hierarchy the course teaches; the agent object echoes `reportsTo` so read it back to confirm the report points at its manager.
...
In the CEO course the CEO creates tasks on its heartbeats after you approve its strategy, and assigns each one to whoever should own it: to **itself** for the first few, and to a **report** once it has hired one (`assigneeAgentId` = the report's agent id). You create an issue by hand only when you want to direct one specific piece of work. Both paths use the same API below. (Paperclip's API calls these "issues"; the dashboard and this course call them "tasks".)

**The workforce spine (Scenario 4, verified live end-to-end).** This is the delegation arc the reworked course now requires you to drive, and every link below was confirmed on a live install: (1) the CEO fills the board and files a `hire_agent` approval; (2) the human approves it; (3) you register the report with `reportsTo` = the CEO's id and a model adapter (`claude_local`, keyless); (4) the CEO (or you, on its behalf) creates a task with `assigneeAgentId` = the report's id, born `todo`; (5) you fire **the report's** heartbeat (`paperclipai heartbeat run -a <report-id>`), and the report moves the task `todo -> in_progress -> done`, leaving its result as a comment authored by the report's id. The human watches a hire they approved do real work. The disposition mechanics below apply to the report exactly as to the CEO; a report is just an agent with a manager.
...
**A budget only moves when the run is metered (priced per token), and no local-CLI path is.** Verified live across all three local adapters (`claude_local` on a Claude Code subscription, `codex_local` on a ChatGPT login, `gemini_local` on a Gemini CLI login): each writes a real `cost_events` row with true input/output token counts, but `cost_cents` is **0**, so `spentMonthlyCents` never climbs and the cap never trips. The whole course runs fine on this keyless path. **There is no local-CLI runtime on which the human watches the budget move.** Metering real cost would need an agent on a genuine pay-per-token API key that Paperclip can price, a separate billing setup this brief has not verified. So teach the budget honestly: set the cap, state the rule (80% warn, 100% pause), and show the recorded spend is `$0` on a subscription. Do not promise a live climb-and-pause on the local-CLI path.

The cap, warning, and auto-pause mechanism is real (at **80%** Paperclip warns; at **100%** the agent is paused and no more heartbeats fire); it simply has nothing to count when the run is unpriced.
